...
executeDbDiagnostics <- function(connectionDetails,
  ...
  for (k in 1:length(dataDiagnosticsSettingsList)) {
    ...
    checkmate::assertIntegerish(studySpecs$studyStartDate, null.ok = TRUE)
    checkmate::assertIntegerish(studySpecs$studyEndDate, null.ok = TRUE, min.chars = 6, max.chars = 6)
  ...
  for (i in 1:dbNum) {
    ...
    for (k in 1:length(dataDiagnosticsSettingsList)) {
      ...
      personsInBirthYearRange <- dbProfile %>%
        ...
        select(countValue) %>%
        mutate(
          statistic = "propInAgeRange",
          spec = case_when(
            is.null(studySpecs$maxAge) && !is.null(studySpecs$minAge) ~ paste("> age", studySpecs$minAge),
            ...
            is.null(studySpecs$maxAge) && is.null(studySpecs$minAge) ~ paste("age", minAge, "- age", maxAge)
          ),
          evaluateThreshold = 1
        )
      ...
      personsWithAgeAtFirstObs <- dbProfile %>%
        ...
        select(countValue) %>%
        mutate(
          statistic = "propWithAgeAtFirstObs",
          spec = case_when(
            is.null(studySpecs$maxAge) && !is.null(studySpecs$minAge) ~ paste("> age", studySpecs$minAge),
            !is.null(studySpecs$maxAge) && is.null(studySpecs$minAge) ~ paste("< age", studySpecs$maxAge),
            !is.null(studySpecs$maxAge) && !is.null(studySpecs$minAge) ~ paste("age", studySpecs$minAge, "- age", studySpecs$maxAge),
      ...
      personsWithGenderCriteria <- dbProfile %>%
        ...
        select(countValue) %>%
        mutate(
          statistic = "propWithGenderCriteria",
      ...
      personsWithRaceCriteria <- dbProfile %>%
        ...
        select(countValue) %>%
        mutate(
          statistic = "propWithRaceCriteria",
      ...
      personsWithEthnicityCriteria <- dbProfile %>%
        ...
        select(countValue) %>%
        mutate(
          statistic = "propWithEthnicityCriteria",
      ...
      personsWithLongitudinalCriteria <- dbProfile %>%
        ...
        select(countValue) %>%
        mutate(
          statistic = "propWithLongitudinalCriteria",
      ...
      personsWithRequiredDomain <- dbProfile %>%
        ...
        select(countValue) %>%
        mutate(
          statistic = "propWithRequiredDomain",
      ...
      personsWithConditionCriteria <- dbProfile %>%
        ...
        select(countValue) %>%
        mutate(
          statistic = "propWithConditionCriteria",
          spec = case_when(
            desiredCondition == 1 ~ "Conditions desired",
      ...
      personsWithDrugCriteria <- dbProfile %>%
        ...
        select(countValue) %>%
        mutate(
          statistic = "propWithDrugCriteria",
          spec = case_when(
            desiredDrug == 1 ~ "Drugs desired",
      ...
      personsWithDeviceCriteria <- dbProfile %>%
        ...
        select(countValue) %>%
        mutate(
          statistic = "propWithDeviceCriteria",
          spec = case_when(
            desiredDevice == 1 ~ "Devices desired",
      ...
      personsWithMeasurementCriteria <- dbProfile %>%
        ...
        select(countValue) %>%
        mutate(
          statistic = "propWithMeasurementCriteria",
          spec = case_when(
            desiredMeasurement == 1 ~ "Measurements desired",
      ...
      personsWithDeathCriteria <- dbProfile %>%
        ...
        select(countValue) %>%
        mutate(
          statistic = "propWithDeathCriteria",
          spec = case_when(
            desiredDeath == 1 ~ "Death domain desired",
      ...
      personsWithProcedureCriteria <- dbProfile %>%
        ...
        select(countValue) %>%
        mutate(
          statistic = "propWithProcedureCriteria",
          spec = case_when(
            desiredProcedure == 1 ~ "Procedures desired",
      ...
      personsWithObservationCriteria <- dbProfile %>%
        ...
        select(countValue) %>%
        mutate(
          statistic = "propWithObservationCriteria",
          spec = case_when(
            desiredObservation == 1 ~ "Observations desired",
      ...
      personsWithIPCriteria <- dbProfile %>%
        ...
        select(countValue) %>%
        mutate(
          statistic = "propWithIPCriteria",
          spec = case_when(
            requiredIP == 1 && desiredIP == 1 ~ "Inpatient visits required and desired",
          ...
          evaluateThreshold = case_when(
            requiredIP == 1 && desiredIP == 1 ~ 1,
      ...
      personsWithOPCriteria <- dbProfile %>%
        ...
        select(countValue) %>%
        mutate(
          statistic = "propWithOPCriteria",
          spec = case_when(
            requiredOP == 1 && desiredOP == 1 ~ "Outpatient visits required and desired",
      ...
      personsWithERCriteria <- dbProfile %>%
        ...
        select(countValue) %>%
        mutate(
          statistic = "propWithERCriteria",
          spec = case_when(
            requiredER == 1 && desiredER == 1 ~ "Emergency Room visits required and desired",
      ...
      personsWithRequiredTargetConcepts <- dbProfile %>%
        ...
        select(countValue) %>%
        mutate(
          statistic = "propWithRequiredTargetConcepts",
          spec = target,
          evaluateThreshold = 2
        )
      ...
      if (is.null(studySpecs$comparatorConceptIds)) {
        personsWithRequiredComparatorConcepts <- data.frame(
          statistic = "propWithRequiredComparatorConcepts",
          countValue = NA,
      ...
      if (is.null(studySpecs$indicationConceptIds)) {
        personsWithRequiredIndicationConcepts <- data.frame(
          statistic = "propWithRequiredIndicationConcepts",
          countValue = NA,
          spec = NA, # Revised from NULL to NA since you cannot declare a column with a NULL value as the only value in the data frame.
          evaluateThreshold = 0
        )
      ...
      if (is.null(studySpecs$outcomeConceptIds)) {
        personsWithRequiredOutcomeConcepts <- data.frame(
          statistic = "propWithRequiredOutcomeConcepts",
          countValue = NA,
          spec = NA, # Revised from NULL to NA since you cannot declare a column with a NULL value as the only value in the data frame.
          evaluateThreshold = 0
        )
